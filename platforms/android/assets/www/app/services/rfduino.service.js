...
//		$window.rfduino = mock_rfduino;
		...
		function initializeDevice() {
			...
			.then(function() {
				function uuidView(detectedDevice) {
					return detectedDevice.uuid;
				}
				
				var deviceIndex = deviceList.map(uuidView).indexOf(lastDevice.uuid);
				var deviceFound = deviceIndex > -1;
		...
		function discoverDevices(deviceList) {
			...
			if ($window.rfduino) {
				...
				isEnabled().then(function() {
    				...
    				$timeout(function() {
    					if (!error) {
    						//TODO: Check if found lastDevice, ask if connect
